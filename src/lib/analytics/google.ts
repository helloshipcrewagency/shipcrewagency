...
// ---- Search Console: URL Inspection (index status) -------------------------
export type UrlInspection = { coverageState: string; verdict: string };
...
export async function gscSubmitSitemap(sitemapUrl: string): Promise<boolean> {
  const token = await getAccessToken();
  const site = encodeURIComponent(process.env.GSC_SITE_URL as string);
